...
module.exports = async function (context, req) {
    ...
    try {
        ...
        const { accessKey, secretKey, region, service, bucket, method, path, payload } = req.body;

        // Validate required parameters
        if (!accessKey || !secretKey || !region || !service || !bucket || !method || !path) {
            throw new Error("Missing required parameters.");
        }

        // Generate AWS Signature
        const signature = generateAWSSignature({ accessKey, secretKey, region, service, bucket, method, path, payload });
